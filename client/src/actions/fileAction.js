  ...
  async (dispatch) => {
    try {
      ...
      const { data } = await axios.get(
        `https://jellyfish-app-4v9we.ondigitalocean.app/auth/me/uploadedfiles?page=${currentPage}&keyword=${keyword}`
      );
      ...
      if (data.success === true) {
        ...
        dispatch({
          type: GET_FILES_SUCCESS,
          payload: "You need to add the data",
        });
      }
    } catch (error) {
  ...
  async (dispatch) => {
    try {
      ...
      const { data } = await axios.post(
        `https://jellyfish-app-4v9we.ondigitalocean.app/file/sendfile`,
        formData,
        cid,
        isCert,
        config
      ...
      fetch("https://jellyfish-app-4v9we.ondigitalocean.app/file/getAllFiles")
